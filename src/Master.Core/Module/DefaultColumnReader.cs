...
namespace Master.Module
{
    public class DefaultColumnReader : IColumnReader, ITransientDependency
    {
        ...
        /// <returns></returns>
        public virtual async Task Read(ColumnReadContext context)
        {
            try
            {
                ...
                if (columnInfo.ColumnType == ColumnTypes.Images || columnInfo.ColumnType == ColumnTypes.Files)
                {
                    ...
                    {
                        var file = await _fileManager.GetByIdFromCacheAsync(int.Parse(id));
                        if (file != null)
                        {
                            files.Add(file);
                        }
                ...
                if (columnInfo.ColumnType == ColumnTypes.Select && !string.IsNullOrEmpty(columnInfo.DictionaryName))
                {
                    ...
                    //var enumType = TypeFinder.Find(o => o.FullName == columnInfo.DictionaryName);
                    //if (enumType.Length > 0)
                    //{
                    //    dic=Common.EnumHelper.ToDictionary(enumType[0]).ToDictionary(o => o.Key.ToString(), o => o.Value);
                    //}
                    //else
                    //{
