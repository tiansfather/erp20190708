...
namespace Master.Module
{
    public class ModuleInfoManager : DomainServiceBase<ModuleInfo, int>, IModuleInfoManager
    {
        ...
        /// <returns></returns>
        private string BuildDynamicSelectString<TEntity>(IEnumerable<ColumnInfo> columnInfos)
        {
            var propertyStrList = new List<string>();
            var _valuePathParser = Resolve<IValuePathParser>();
            //如果有状态列，则直接加入
            if (typeof(IHaveStatus).IsAssignableFrom(typeof(TEntity)) && !propertyStrList.Contains("Status"))
            {
                propertyStrList.Add("Status");
            }
            //直接加入Id列
