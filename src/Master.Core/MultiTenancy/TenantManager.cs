...
namespace Master.MultiTenancy
{
        ...
        IEventHandler<EntityDeletedEventData<Edition>>
    {
        ...
        {
            //触发用户权限缓存清除
            Resolve<IEventBus>().Trigger(new UserGrantedPermissionIndicator { TenantId = tenant.Id });
        ...
        {
            //触发用户权限缓存清除
            Resolve<IEventBus>().Trigger(new UserGrantedPermissionIndicator { TenantId = tenantId });
