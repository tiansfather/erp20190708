...
namespace Master.Finance
{
    ...
    {
        [InterColumn(ColumnName = "状态", Sort = 0,ColumnType =Module.ColumnTypes.Select, DictionaryName = "Master.Finance.InvoiceStatus",Templet ="{{d.invoiceStatus_display}}", EnableDataFilter = true)]
        public InvoiceStatus InvoiceStatus { get; set; }
        [InterColumn(ColumnName ="发票申请单编号",Sort =1)]
        public string SheetSN { get; set; }
        [InterColumn(ColumnName = "发票类别", Sort = 2, EnableDataFilter = true)]
        ...
        [InterColumn(ColumnName = "品牌单位名称", Sort = 8, EnableDataFilter = true)]
        public string SellUnitName { get; set; }
        [InterColumn(ColumnName = "申请者", DisplayPath = "Unit.UnitName", Templet = "{{d.unitId_display||'/'}}", Sort = 9, EnableDataFilter = true)]
        ...
        [InterColumn(ColumnName = "开户银行", Sort = 14, EnableDataFilter = true)]
        public string BuyUnitBank { get; set; }
        [InterColumn(ColumnName = "开户行账号", Sort = 15)]
        public string BuyUnitBankAccount { get; set; }
        [InterColumn(ColumnName = "申请操作", Sort = 16, EnableDataFilter = true)]
