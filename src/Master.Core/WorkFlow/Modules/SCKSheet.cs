...
{
    [InterModule("票劵出库单查询", BaseType = typeof(FlowSheet), GenerateDefaultButtons = false)]
    public class SCKSheet: FlowSheet
    {
        [InterColumn(ColumnName = "出库单编号", Templet = "<a dataid=\"{{d.id}}\" class=\"layui-btn layui-btn-xs layui-btn-normal\" buttonname=\"单据\" params=\"{&quot;btn&quot;:[]}\"   buttonactiontype=\"Form\" buttonactionurl=\"/FlowSheet/SheetView\" onclick=\"func.callModuleButtonEvent()\">{{d.sheetSN}}</a>", Sort = 1)]
        public override string SheetSN { get => base.SheetSN; set => base.SheetSN = value; }
        [InterColumn(ColumnName = "收货代理", DisplayPath = "Unit.UnitName", Templet = "{{d.unitId_display||'/'}}", Sort = 2, EnableDataFilter = true)]
        ...
        [InterColumn(ColumnName = "应收货款", ValuePath = "Property", ColumnType = Module.ColumnTypes.Number, Sort = 5)]
        [NotMapped]
