...
namespace Master.Domain
{
        ...
        where TEntity : class, IEntity<TPrimaryKey>, new()
    {
        ...
        /// <returns></returns>
        //public IQueryable GetGroupedFieldQuery(string field,string keyword = "",int take=50)
        //{
        //    var query = GetAll().Where($"{field}!=null");
        //    if (!string.IsNullOrEmpty(keyword))
        //    {
        //        query = query.Where($"{field}.Contains(\"{keyword}\")");
        //    }

        //    return query
        //        .GroupBy(field).Take(take).Select("Key");
