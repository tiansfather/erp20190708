...
namespace Master.WorkFlow.Domains
{
    public class FlowRuntime
    {
        ...
        public string NodeConfluence(string nodeId, Tag tag)
        {
            ...
            {
                if (tag.Taged == TagState.No)  //只要有一个不同意，那么流程就结束
                {
                    res = TagState.No.ToString("D");
                }
                else if (tag.Taged == TagState.Ok)
                {
        ...
        public void MakeTagNode(string nodeId, Tag tag)
        {
            foreach (var item in Nodes)
            {
                if (item.Key == nodeId)
                {
                    ...
                    item.Value.setInfo.Taged = Convert.ToInt32(tag.Taged);
