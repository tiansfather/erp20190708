...
var app = new Vue({
    ...
    methods: {
        ...
        changeLanguage: function () {
            abp.localization.languages.forEach(el => {
                if (el.name != abp.localization.currentLanguage.name) {
                    top.location.href = '/AbpLocalization/ChangeCulture?cultureName=' + el.name + '&returnUrl=/';
                }
            })
        },
        ...
        judgeExpireDate() {
            var expireDate = abp.session.tenant.expireDate;
            if (!expireDate)return 
            var difference = Math.ceil((new Date(expireDate) - new Date()) / (24 * 60 * 60 * 1000))
            const firstRemind = 15; const lastRemind = 10;
            function openMsg(type) {
                var str = type != -1 ? `您的账套再过${difference}天到期` : '您的账套已经过期';
                var obj = {
                    title: '权益到期提醒',
                    content: `&emsp;&emsp;亲爱的用户，${str}，到期后无法登陆使用，为了避免给您带来不便或造成损失，请通知你们的出纳立即付款。
                    <br> 付款方式:  台州市汉德软件有限公司
                    <br> 开户行：浙江泰隆商业银行台州黄岩支行
                    <br> 账号：3301180120100027924
                    <br> &emsp;&emsp;付款后通知我们技术延续服务，本条信息纯属不得已而发送，请谅解。`,
                    btn: [],
                    closeBtn: 0,
                    offset: 'rb',
                    shade: 0,
            ...
            if (difference < 0) {
                //已经到期，后台已经判断住了，进不去
                openMsg(-1)
            } else if (lastRemind > difference&&difference >= 0) {
                //马上到期
                openMsg(0);
            } else if (firstRemind > difference&&difference > lastRemind) {
                //有一个缓冲日期，5到15天
                openMsg(1);
            }
        }
