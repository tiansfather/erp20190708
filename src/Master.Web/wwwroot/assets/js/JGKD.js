...
config.ready = function () {
    ...
    layui.use('formSelects', function () {
        ...
        function getUnitNameArr(result) {
            //分类
            var types = {};
            var arr = [];
            result.forEach(elm => {
                if (!elm.unitTypeDisplayName) {
                    elm.unitTypeDisplayName = '未分类'
                }
                if (!types[elm.unitTypeDisplayName]) {
                    types[elm.unitTypeDisplayName] = [];
                }
                types[elm.unitTypeDisplayName].push({ name: elm.unitName, value: elm.unitName })
            })
            for (var i in types) {
                arr.push({ name: i, type: "optgroup" })
                types[i].forEach(elm => {
                    arr.push(elm)
                })
            }
        ...
        function useFunction(app) {
            function fBeforeSuccess(selectId, searchVal, result) {//返回下拉框数据
                ...
                var StandardPartName = abp.setting.get("MES.StandardPartName").split(',')
                var aDefaultMsg = StandardPartName.length ? StandardPartName : ["动模", "定模", "滑块", "镶块"];
                if (selectId == 'partName' && !searchVal) {//当零件下拉框读取到内容为空时,并且不是进行搜索操作时，会自动添加以上信息
                    aDefaultMsg.forEach(function (defaultMsg) {
                        resultArr.push(new Arr(defaultMsg))
                    })
                }
                    ...
                    function fGetSUrl(sId) {
                        switch (sId) {
                            ...
                            case 'unitName':
                                ...
                                //sUrl ="/api/services/app/MESUnit/GetAllUnitByProcessTypeName"
                                break;
                            case 'equipmentSN':
        ...
        //    app.currentItem.inner = data.elem.checked;
        //    app.currentItem.equipmentSN = '';
        //    //layui.layer.tips('自动清空加工点', data.othis)
        //    //if (data.elem.checked) {
        ...
        //});
        form.on("submit(submitSend)", function () { doSend() });
        form.on("submit(submitPreview)", function () { doPreview() });
        ...
        app = new Vue({
            ...
            data: {
                currentItem: {
                    partNum: 1, files: [], emergency: false, partSpecification: '', partSN: '', taskInfo: '参考附件', feeType: 1, feeFactor: '', price: 0, jobFee: 0, estimateHours: '', sheetFile: {}, requireDate: '', appointDate: '', poster: '', projectCharger: '', craftsMan: '', verifier: '', checker: '', equipmentSN: '', unitName: '', inner: false, cha: false, xiu: false, partName: '', projectSN: '', reason: '', processTaskDetails: [], multi: false, processTaskParts: [{}]
                },
            ...
            computed: {
                multiItems: function () {
                    ...
                    return [
                        { type: 'text', title: '零件名称', verify: '', key: 'partName', width: '17%' },
                        { type: 'select', title: '零件编号', verify: '', key: 'partSN', options: options, onChange: onChange ,width:'22%'},
                        { type: 'text', title: '规格', verify: '', key: 'partSpecification', width: '15%' },
                        { type: 'text', title: '数量', verify: '', key: 'partNum', width: '13%', defaultData:1 },
                        {
                            type: 'upload', title: '零件图片', key: 'upload', width: '7.5%', uploadFileName:'readonlyFileName',
                            onChange: function (ev, index) {
                                app.currentItem.processTaskParts[index].sheetFile = ev;
                                //app.currentItem.processTaskParts[index].readonlyFileName = ev.fileName;
                            }
                        }, { type: 'readonly', title: '文件名称', key: 'readonlyFileName', width: '12%',},
                    ]
                },
                ...
                mustFull: function () {//预约日期，要求完成时间，预计工时必填
                    return abp.setting.getBoolean('MES.JGKDFullMode')
                },
                disabled: function () {
            ...
            methods: {
                ...
                swtichEmergency: function (bol) {       //加急按钮点击后触发后函数，接到从组件传来的布尔值
                    this.currentItem.emergency = bol;
                ...
                fSubmitForm: function (fn) {//提交函数
                    ...
                    if (this.ConfirmProcessFlag) {
                        ...
                        func.runAsync(abp.services.app.processTask.submitProcessTask(this.currentItem).done(function (data) {
                            var msg = data.processSN ? (abp.setting.getBoolean('MES.MustConfirmProcess') && !app.currentItem.processSN ? '已审核成功' : '已成功开单或修改') : '信息保存成功';
                            app.currentItem.files = [];
                            app.currentItem.processTaskParts = [];
                            $.extend(true, app.currentItem, data)
                            //返回的是加工单id
                ...
                loadItem: function (taskid) {//有taskid代表是进入编辑页
                    var that = this;
                    //厂内开关默认值
                    if (abp.setting.getBoolean('MES.DefaultSourceInner')) {
                        that.currentItem.inner = true;
                    }
                    //console.log(taskid)
                    if (!taskid) {
...
function permission(fn,str) {
    if (abp.setting.getBoolean("MES.MustConfirmProcess")) {
        ...
        if (app.currentItem.inner || app.currentItem.unitName) {
            ...
            abp.message.error("请先填写加工点或设备进行开单，再点击" + str);
            return false;
...
function doSend() {
    function goahead() {
        ...
        function failF(err) {
            if (err.code != -1) {
                ...
                (function loopFunc() {
                    abp.services.app.mESUnit.getUnitTenantInfoByName(app.currentItem.unitName).done(res => {
                        if (res.tenantBinded) {
                            //跳出循环
                            layer.close(index);
                        } else {
        ...
        abp.services.app.mESUnit.findUnitOpenId(app.currentItem.supplierId, 'ReceiveOuterTask', { error: failF, abpHandleError: false }).done(function (res) {
            if (!window._sendDoneFn) {
                //给jq添加发送成功后的回调，只在子iframe中用到，不想再放出来一个全局变量
                window._sendDoneFn = function _sendDoneFn() {
                    app.currentItem.sendProcessor = true;
                }
            }
            $('body').append('<iframe style="display:none;" src="/Home/Show?name=../MES/SheetView&taskid=' + app.currentItem.id + '&sendUnit=1&donefn=_sendDoneFn">')
            layer.msg('发送中，请稍后', {
                icon: 16,
                shade: 0.5,
                time: 0
    ...
    if (app.currentItem.inner) {
        ...
        if (app.currentItem.sendProcessor) {
            abp.message.confirm(L('您已经发送过此任务给\"' + app.currentItem.unitName + '\"<br>是否再次发送？'), function () {
...
function doSubmitProcessQuote() {
    var $btn = $(getEvent().target);
        //询价按钮触发事件
    
    if (app.currentItem.inner) {
        return layer.msg('厂内无法询价');
    } else if (app.currentItem.processSN) {
        return layer.msg('已开单无法询价');
    } else if (app.currentItem.unitName) {
        return layer.msg('已选择加工点无法询价');
    }
    app.fSubmitForm(fn);
    function fn() {
        $btn.attr('buttonactionurl', $btn.attr('oriurl') + `?taskid=${app.currentItem.id}`)
        func.callModuleButtonEvent($btn)
    }
}
//加载零件的加工路线
