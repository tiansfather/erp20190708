...
CanvasImg.prototype = {
    goRotate: function (direction) {
        ...
        var step = this.image.getAttribute('step');
        if (step == null) {
            step = min_step;
        }
        if (direction == 'right') {
        ...
        this.image.setAttribute('step', step);
        this._rotate(step);
        this.url = this.canvas.toDataURL('image/jpg');
        return this.url;
    },
    _rotate: function (step) {
    ...
    _getCanvas: function (width, height) {
        this.canvas = $('<canvas style="display:none;"/>').appendTo($('body'))[0];
    }
}