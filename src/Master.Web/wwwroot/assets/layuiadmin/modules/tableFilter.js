    ...
    var mod = {
        ...
        render: function (myTable) {
            var _this = this,
                ...
                filterItems = myTable.filter?myTable.filter.items||['data','condition','editCondition','excel']:['data','condition','editCondition','excel'],
            ...
            if (!initFilter || isFilterReload[myTable.id]) {
                ...
                console.log(myTable.where);
                //where_cache[myTable.id] = {}
                //modi20190613 解决当前台where查询改变后插件中不缓存查询条件的问题
            ...
            if ($('#soul-filter-list' + tableId).length == 0) {
                ...
                //modi 20190613 将排序也作为配置项
                var soulFilterList = [],
                    filterItemsHtml = {
                        sort:'<li class="soul-sort" data-value="asc" ><i class="soul-icon soul-icon-asc"></i> 升序排列 </li><li class="soul-sort" data-value="desc"  style="border-bottom: 1px solid #e6e6e6"><i class="soul-icon soul-icon-desc"></i> 降序排列 </li>',
                ...
                $('#main-list' + tableId + ' .soul-dropList').on('mouseover', function (e) {
                    if ($('#soul-dropList' + tableId).is(':visible') && !$('#soul-dropList' + tableId).hasClass('fadeOut2Left')) {
                        return false;
                    }
                ...
                $('#main-list' + tableId + ' .soul-condition').on('mouseover', function (e) {
                    if ($('#soul-condition' + tableId).is(':visible') && !$('#soul-condition' + tableId).hasClass('fadeOut2Left')) {
                        return false;
                    }
                    _this.hideColumns(myTable);
                    _this.hideDropList(myTable);
            ...
            if ($tableHead.find('.soul-table-filter').length > 0) {
                ...
                if (filterItems.indexOf('data')!=-1) {
                    if (typeof myTable.url != 'undefined' && myTable.page) {
                        ...
                        datas['filterColumns'] = JSON.stringify(columnField);//modi by tiansfather
                        $.ajax({
                            url: url,
                            data: datas,
                            dataType: 'json',
                            success: function (result) {
                                var result = result.data;//modi by tiansfather
                                var uls = [];
        ...
        showConditionBoard: function (myTable) {
            ...
            function showColums(obj) {
                ...
                $('#soul-bf-column' + tableId)
                    ...
                    .removeClass().addClass('fadeIn2Up animated')
                ...
                switch (mode) {
                    case 'in':
                        ...
                        $('#soul-dropList' + tableId).css({'top': top, 'left': left})
                            .show().removeClass().addClass('fadeIn2Up animated');
                        setTimeout(function () {
                            $('#soul-dropList' + tableId + '>.filter-search>input').focus() // 聚焦搜索框
                        }, 1);
                    ...
                    case 'date':
                        ...
                        _this.showDate(myTable, field, filterSo, "fadeIn2Up", top, left, "down", refresh);
                        break;
                    case 'condition':
                        $(obj).hide();
        ...
        , hideColumns: function (myTable, animate) {
            ...
            $('#soul-columns' + tableId).removeClass().addClass('fadeOut2Left animated')
            if (columnsTimeOut) {
                clearTimeout(columnsTimeOut)
            }
        ...
        , hideDropList: function (myTable, animate) {
            var $table = $(myTable.elem),
                tableId = $table.attr('id');
            $('#soul-dropList' + tableId).removeClass().addClass('fadeOut2Left animated')
            if (dorpListTimeOut) {
                clearTimeout(dorpListTimeOut);
            }
            if (typeof animate == 'undefined' || animate) {
        ...
        , hideCondition: function (myTable, animate) {
            var $table = $(myTable.elem),
                tableId = $table.attr('id');
            $('#soul-condition' + tableId).removeClass().addClass('fadeOut2Left animated')
            if (conditionTimeOut) {
                clearTimeout(conditionTimeOut);
            }
            if (typeof animate == 'undefined' || animate) {
        ...
        , hideBfPrefix: function (myTable, animate) {
            var $table = $(myTable.elem),
                tableId = $table.attr('id');
            $('#soul-bf-prefix' + tableId).removeClass().addClass('fadeOut2Down animated')
            if (bfColumnTimeOut) {
        ...
        , hideBfColumn: function (myTable, animate) {
            var $table = $(myTable.elem),
                tableId = $table.attr('id');
            $('#soul-bf-column' + tableId).removeClass().addClass('fadeOut2Down animated')
            if (bfColumnTimeOut) {
                clearTimeout(bfColumnTimeOut);
            }
            if (typeof animate == 'undefined' || animate) {
        ...
        , hideBfType: function (myTable, animate) {
            var $table = $(myTable.elem),
                tableId = $table.attr('id');
            $('#soul-bf-type' + tableId).removeClass().addClass('fadeOut2Down animated')
            if (bfCond1TimeOut) {
        ...
        , updateWhere: function (myTable, filterSo) {
            ...
            //where_cache[myTable.id] = myTable.where;
            //modi 20190613 修正查询条件问题
            where_cache[myTable.id].filterSos = myTable.where['filterSos'];
            ...
            $bottomCondition.find('.item-value').on('click', function (e) {
                ...
                switch (mode) {
                    ...
                    case 'date':
                        ...
                        _this.showDate(myTable, field, filterSo, "fadeIn2Up", top, left, "up", true);
                        break;
                    default:
                        _this.hideDropList(myTable);
                        if (conditionTimeOut) {
