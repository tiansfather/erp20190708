...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  Class.prototype.events = function(){
            ...
            openPanel({
              ...
              ,done: function(panel, list){
                list.on('click', function(){
                    ...
                    var data = $.extend(params, options.where);
                    $.ajax({
                        url: options.url, data: data, dataType: 'json'
                        , headers: options.headers || {}, success: function (res) {
                            table.exportFile(options.id, res.data, type);
                        }
                    });
    ...
    //})
        th.find(ELEM_SORT + ' .layui-edge ').on('click', function (e) {
      var othis = $(this)
      ,index = othis.index()
    ...
    ,gridExpand = function(hide){
      var othis = $(this)
      ,elemCell = othis.children(ELEM_CELL);
        //modi 2019/08/22 totar行不触发事件，导致按钮列过长不显示展开图标
      //if(othis.data('off')) return; //不触发事件
    ...
    that.layMain.on('scroll', function(){
      ...
      that.layFixed.find(ELEM_BODY).scrollTop(scrollTop);
        ...
        //modi 19.11.15 操作列，宽度不够时，点击偶尔会触发
      //layer.close(that.tipsIndex);
    });
