...
namespace Master
{
        ...
        where TEntity : class, IEntity<TPrimary>, new()
    {
        ...
        /// 
        /// </summary>
        /// <param name="requestGroupFieldPageDto"></param>
        /// <returns></returns>
        public virtual async Task<IEnumerable<dynamic>> GetGroupedField(RequestGroupFieldPageDto requestGroupFieldPageDto)
        {
            
            var result = new List<dynamic>();
            var query=await GetQueryable(requestGroupFieldPageDto);

            if (!string.IsNullOrEmpty(requestGroupFieldPageDto.Query))
            {
                query = query.Where($"{requestGroupFieldPageDto.Field}.Contains(\"{requestGroupFieldPageDto.Query}\")");
            }
            result.AddRange((await
                query
                .GroupBy(requestGroupFieldPageDto.Field)                
                .Select("Key")
                .Skip((requestGroupFieldPageDto.Page - 1) * requestGroupFieldPageDto.Limit)
                .Take(requestGroupFieldPageDto.Limit)
                .ToDynamicListAsync()).Distinct());
        ...
        /// </summary>
        /// <returns></returns>
        public virtual async Task<IEnumerable<object>> GetAllByKeyword(string keyword,int take=1000)
        {
            var query = await BuildKeywordQueryAsync(keyword, Manager.GetAll());
            return (await query.Take(take).ToListAsync())
                .Select(o => ResultConverter(o));
        }
        ...
        /// 设置实体的属性
        /// </summary>
        /// <param name="propertySetDto"></param>
        /// <returns></returns>
        public virtual async Task SetPropertyValue(PropertySetDto propertySetDto)
        {
            if (propertySetDto.Ids.Count() != propertySetDto.Values.Count())
            {
                throw new UserFriendlyException("参数错误");
            }
            if(!typeof(IHaveProperty).IsAssignableFrom(typeof(TEntity)))
            {
                throw new UserFriendlyException("类型不支持属性设置");
            }
            var entities = await Manager.GetListByIdsAsync(propertySetDto.Ids);
            for(var i = 0; i < entities.Count(); i++)
            {
                var entity = entities.ElementAt(i);
                (entity as IHaveProperty).SetPropertyValue(propertySetDto.PropertyName, propertySetDto.Values.ElementAt(i));
            }
        }
        ...
        /// <returns></returns>
        public virtual async Task<IEnumerable<object>> GetPropertyValue(IEnumerable<TPrimary> primaries, string propertyName)
        {
            if (!typeof(IHaveProperty).IsAssignableFrom(typeof(TEntity)))
            {
                throw new UserFriendlyException("类型不支持属性设置");
            ...
            var entities = await Manager.GetListByIdsAsync(primaries);
            return entities.Select(o => (o as IHaveProperty).GetPropertyValue(propertyName));
        }
        #endregion
        public class PropertySetDto
        {
            public IEnumerable<TPrimary> Ids { get; set; }
